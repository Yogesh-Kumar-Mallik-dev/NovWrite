...
export default function CreateSceneScreen() {
  ...
  const initialChapterId =
    params.chapterId ||
    mobileStore.getState().activeChapterId ||
    chapters[0]?.id ||
    "";
  ...
  const handleSave = () => {
    ...
    const newScene = mobileStore.createScene(
      chapterId,
      finalTitle,
      tWords,
      synopsis.trim(),
    ...
    if (status !== "DRAFT") {
      mobileStore.updateScene(newScene.id, { status });
    }

    mobileStore.selectScene(newScene.id);
    router.back();
  };
        ...
        >
          ...
          <View className="mb-5">
            ...
            <TextInput
              ...
              placeholder="e.g. Scene 1: Arrival at Celestial Summit"
