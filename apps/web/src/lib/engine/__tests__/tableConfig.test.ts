...
describe("BLOCK_WORLD_TABLE_CONFIG_001: Table Column Customization Engine", () => {
  ...
  it("should generate correct available columns for global and per-blueprint views", () => {
    ...
    assert.ok(
      charCols.some((c) => c.id === "prop:gender" && c.category === "dynamic"),
    );
    assert.ok(
      charCols.some(
        (c) => c.id === "prop:rarity" && c.fieldType === "VALUE_TYPE",
      ),
    );
    assert.ok(charCols.some((c) => c.id === "prop:attack" && c.unit === "pts"));
    assert.ok(
      charCols.some(
        (c) => c.id === "formula:combat_power" && c.category === "formula",
      ),
    );
  });
  ...
  it("should correctly format various table cell types", () => {
    ...
    assert.equal(
      formatTableCellValue("blueprintName", mockEntity).text,
      "Character Archetype",
    );
    assert.equal(
      formatTableCellValue("lastMutatedSeqNumber", mockEntity).text,
      "#5",
    );

    // Dynamic number with unit
    const attackCell = formatTableCellValue(
      "prop:attack",
      mockEntity,
      mockCharacterBlueprint,
    );
    assert.equal(attackCell.text, "1500 pts");

    // Dynamic enum/string
    const genderCell = formatTableCellValue(
      "prop:gender",
      mockEntity,
      mockCharacterBlueprint,
    );
    ...
    // Dynamic value type
    const rarityCell = formatTableCellValue(
      "prop:rarity",
      mockEntity,
      mockCharacterBlueprint,
    );
    ...
    // Formula cell
    const powerCell = formatTableCellValue(
      "formula:combat_power",
      mockEntity,
      mockCharacterBlueprint,
    );
    ...
    const cultCell = formatTableCellValue(
      "prop:cultivation",
      mockEntity,
      mockCharacterBlueprint,
    );
  ...
  it("should handle null/missing properties and format array fields gracefully (regression)", () => {
    const emptyEntity: TableEntityItem = {
      id: "ent-empty",
      blueprintId: "bp-wanderer",
      blueprintName: "Wanderer",
    ...
    // Missing property should return dash "-"
    const missingCell = formatTableCellValue(
      "prop:unknown_stat",
      emptyEntity,
      mockCharacterBlueprint,
    );
    assert.equal(missingCell.text, "—");

    // Array property
    const arrayEntity: TableEntityItem = {
      id: "ent-array",
      blueprintId: "bp-character",
      blueprintName: "Character Archetype",
      name: "Elder",
      ...
      properties: {
        titles: ["Grandmaster", "Azure Blade", "Sect Founder"],
      },
    };
    const titlesCell = formatTableCellValue("prop:titles", arrayEntity);
    assert.ok(titlesCell.text.includes("Grandmaster"));
  });
});
