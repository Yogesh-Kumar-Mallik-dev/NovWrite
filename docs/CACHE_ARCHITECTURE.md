...
| **2. State Snapshot**      | `novwrite:v1:proj:{projId}:state:seq:{sequenceNumber}`     | Pre-folded point-in-time universe entity states & computed math  | Protobuf / LZ4 JSON | 24 Hours               | Cascading Eviction on Event Edit        |
| **3. Schema & Rules**      | `novwrite:v1:proj:{projId}:schema:blueprints`              | Compiled 1st & 2nd class blueprints and categorical ENUM strings | JSON                | 12 Hours               | Write-Through Invalidation              |
| **3.5. Value Type Tables** | `novwrite:v1:proj:{projId}:schema:valuetypes`              | Dual-valued `VALUE_TYPE` options tables with power weights       | JSON / Hash         | 12 Hours               | Invalidate on Blueprint Option Change   |
| **3.6. Formula ASTs**      | `novwrite:v1:proj:{projId}:schema:formulas`                | Pre-tokenized AST trees for live mathematical formula evaluation | JSON                | 12 Hours               | Invalidate on Blueprint Change          |
