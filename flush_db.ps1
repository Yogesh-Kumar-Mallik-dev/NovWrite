...
if ($dockerComposeCmd) {
    ...
    Write-Host "⚠️  Docker Compose not detected in PATH; attempting direct database reset..." -ForegroundColor Yellow
}
...
if (Get-Command "docker" -ErrorAction SilentlyContinue) {
    try {
        ...
        Write-Host "✅ Redis cache flushed cleanly via Docker container." -ForegroundColor Green
        $redisFlushed = $true
    } catch {}
}
...
if (-not $redisFlushed -and (Get-Command "redis-cli" -ErrorAction SilentlyContinue)) {
    try {
        ...
        Write-Host "✅ Redis cache flushed cleanly via local redis-cli." -ForegroundColor Green
        $redisFlushed = $true
    } catch {}
}

if (-not $redisFlushed) {
    Write-Host "⚠️  Could not contact Redis to flush keys (service may not be running or is starting up)." -ForegroundColor Yellow
}
...
Write-Host "  ✨ Backend Clean Slate Established!" -ForegroundColor Green
